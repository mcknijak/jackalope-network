...
# edit /etc/restic.env: set RESTIC_PASSWORD, B2 keys, and the USB and B2 repo paths

# 3. Initialize each repository (one-time)
...
The recovery path on new hardware:

1. Reinstall Debian and run `bootstrap/debian-setup.sh`.
2. Mount the existing USB backup drive (or pull credentials from a password manager to reach B2).
3. `restic restore latest --target /` from the chosen repo. This puts `/srv`, `/mnt/data`, and the dumps back.
4. `mdadm --assemble --scan` to bring the HDD mirror back if you still have it; otherwise create a new mirror and let restic populate it.
